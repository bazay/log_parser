...
RSpec.describe LogParser::LogItem do
  ...
  describe '.sort_by_unique_views' do
    subject(:sort_by_unique_views) { described_class.sort_by_unique_views(log_items) }

    let(:log_items) { [first_log_item, second_log_item, third_log_item, fourth_log_item] }
    let(:fourth_log_item) { LogParser::LogItem.new(page: '/home', ip_address: '127.0.0.1') }
    let(:expected_result) do
      {
        '/home' => [second_log_item, third_log_item],
        '/contact' => [first_log_item]
